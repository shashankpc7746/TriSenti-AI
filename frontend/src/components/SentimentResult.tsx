...
import { TrendingUp, ChevronDown, Brain, Zap, Copy, Check, Share2 } from 'lucide-react';
...
export function SentimentResult({ sentiment, engine }: SentimentResultProps) {
  ...
  const [resultExported, setResultExported] = useState(false);

  const copyTranscript = async () => {
    if (!sentiment.transcript) return;
    await navigator.clipboard.writeText(sentiment.transcript);
    setTranscriptCopied(true);
    setTimeout(() => setTranscriptCopied(false), 2000);
  };

  const exportResult = async () => {
    const engineLabel = engine === 'hf' ? 'RoBERTa (HuggingFace)' : 'TriSenti Custom Model';
    const lines = [
      '=== TriSenti AI — Sentiment Analysis Result ===',
      ...
      `Audio : ${(sentiment.emotions.audio.score * 100).toFixed(1)}% (${sentiment.emotions.audio.emotion})`,
      `Text  : ${(sentiment.emotions.text.score  * 100).toFixed(1)}% (${sentiment.emotions.text.emotion})`,
      sentiment.probabilities ? ['', '--- Class Probabilities ---',
        ...Object.entries(sentiment.probabilities).map(([k, v]) => `${k.padEnd(8)}: ${(v * 100).toFixed(1)}%`)
      ].join('\n') : null,
      '',
      `Generated by TriSenti AI · ${new Date().toLocaleString()}`,
    ].filter(Boolean).join('\n');

    await navigator.clipboard.writeText(lines);
    setResultExported(true);
    setTimeout(() => setResultExported(false), 2000);
  };
      ...
      <div className={`bg-gradient-to-r ${getColor(sentiment.label)} p-4 sm:p-6 text-white`}>
        <div className="flex flex-col sm:flex-row items-center sm:items-start sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {resultExported ? <Check className="w-4 h-4" /> : <Share2 className="w-4 h-4" />}
              {resultExported ? 'Copied!' : 'Export'}
            </motion.button>
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <motion.button
                ...
                className="flex items-center gap-1 px-2.5 py-1 bg-blue-500/20 hover:bg-blue-500/30 border border-blue-400/30 rounded-lg text-xs font-medium text-blue-300 transition-colors"
              >
